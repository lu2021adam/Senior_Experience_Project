...
// Inspired by https://www.gmass.co/blog/record-audio-mobile-web-page-ios-android/ (last retrieved 5/22/2018)
// Borrowed heavily from https://gist.github.com/rowntreerob/614010bf487d28c4c9f9ddcd0f2d8e70 (last retrieved 5/22/2018)
window.WzRecorder = window.WzRecorder || function WzRecorder(config) {
  config = config || {};
  ...
  var recordingLength = 0;
	...
	this.toggleRecording = function(setTo) {
    // Set `toggle` to the /new/ state.
	  var toggle = typeof setTo !== "undefined" ? !!setTo : !recording;
		toggle ? self.start() : self.stop();
		return toggle;
	};
...
;
(function($) {
  // var file = new File([window.recorder.blob], 'test-awesome.wav', {type: "audio/wav"});

  /**
   * Generic file upload function.
   * @param file
   *  File object created via `new File(...)`
   * @param input
   *  DOMElement object representing a file INPUT tag within a FORM.
   * @param upload_id
   *  String ID corresponding to the Drupal.ajax configuration for this field,
   *  typically the ID of the "Upload" [type=submit] button.
   */
  var uploadFile = function(file, input, upload_id) {
    var form = input.form,
      fd = new FormData(form);

    // It's super important that we use .set() instead of .append() here because
    // otherwise Safari on iOS will not send the later file.
    fd.set(input.name, file, file.name);

    // Simulate a Drupal AJAX submission by cloning the Drupal.ajax params for
    // the input. This includes the appropriate success/error callbacks.
    var options = Drupal.ajax[upload_id].options;
    options.data = fd;
  ...
  var toggleButton = function($button, setTo) {
    // Possibly redundant, but normalizes calls to this function.
    $button = $($button);
    // Set `toggle` to the /new/ state.
    // FALSE = waiting to record, TRUE = recording.
    var toggle = typeof setTo !== "undefined" ? !!setTo : $button.hasClass('record');
    $button.toggleClass('record', !toggle);
    $button.toggleClass('stop', toggle);
    if (!toggle) {
      // Toggle = FALSE, waiting to record.
      $button.attr('value', 'Record');
      if ($button.prop) {
        $button.prop('value', 'Record');
      }
      $button.html('<span class="icon-record">Record</span>');
    }
    else {
      // Toggle = TRUE, recording.
      $button.attr('value', 'Stop/Upload');
      if ($button.prop) {
        $button.prop('value', 'Stop/Upload');
      }
      $button.html('<span class="icon-stop">Stop/Upload</span>');
    }
    return toggle;
  };

  var files = {};
  Drupal.behaviors.webform_html5_uploads = {
    attach: function(context, settings) {
      $('.html5-upload-audio', context).once('webform_html5_uploads').each(function() {
        // Attach is called both when the file field is present (no file uploaded
        // yet) and after a file has been AJAX uploaded (when the REMOVE button is
        // present).
        var $file_container = $(this),
          input_field = $file_container.find('[type=file]').get(0),
          id = $file_container.closest('.form-item').get(0).id,
          upload_id = $file_container.find('[type=submit]').get(0).id;

        $file_container.addClass('webform-html5-uploads');
        if (input_field) {
          // If the file field is present, this is a new upload.
          var $record_button = $('<button class="btn btn-default">').appendTo($file_container);
          var recorder = new WzRecorder({
            onRecordingStop: function (blob) {
              files[id] = new File([blob], 'record.wav', {type: "audio/wav"});
              uploadFile(files[id], input_field, upload_id);
            }
            // , onRecording: function(milliseconds) {
            //   document.getElementById('duration').innerText = milliseconds + 'ms';
            // }
          });
          toggleButton($record_button, false);
          $record_button.click(function (e) {
            e.preventDefault();
            recorder.toggleRecording(toggleButton(this));
          });
        }
        else {
          // If the file field is not present, this file has already been
          // uploaded, so allow it to be played (if we can).
          if (files[id]) {
            $('<audio controls="controls" class="player">')
              .attr('src', URL.createObjectURL(files[id]))
              .prependTo($file_container);
          }
        }
      });
    }
